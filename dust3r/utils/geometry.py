...
def align_poses_batchwise(views, pose_key='camera_pose'):
    """
    Aligns poses sample-wise within a batch.

    For each sample j in the batch, it uses the pose views[0][pose_key][j] as the
    new world origin and transforms all corresponding poses views[i][pose_key][j]
    relative to it.

    Args:
        views (list): A list of dictionaries, where each dictionary represents a view.
        pose_key (str): The key to access the pose tensor in the view dictionary.
                        The pose tensor is expected to be of shape (B, 4, 4).
    """
    # 1. Get the BATCH of reference C2W poses from the first view.
    # This is a tensor of shape (B, 4, 4).
    # 2. Compute the inverse for EACH pose in the batch. `torch.linalg.inv` on
    # a (B, 4, 4) tensor returns a (B, 4, 4) tensor where the j-th matrix
    # is the inverse of the j-th input matrix.
    ref_w2c_batch = torch.linalg.inv(views[0][pose_key])

    # 3. Apply the batch of transformations to all views.
    for view in views:
        # Perform batch matrix multiplication.
        # The j-th pose in current_poses_batch is multiplied by the
        # j-th inverse matrix in ref_w2c_batch.
        # Shape math: (B, 4, 4) @ (B, 4, 4) -> (B, 4, 4)
        view[pose_key] = ref_w2c_batch @ view[pose_key]

    return views

def align_poses(views, pose_key='camera_pose'):
    c2w_to_w2c = inv(views[0][pose_key])
    for view in views:
        view[pose_key] = (c2w_to_w2c @ view[pose_key])   
    return views
...
def camera_coordinates_to_depthmap(camera_coords_map):
    depth_map = camera_coords_map[..., 2]
    return depth_map
...
def unproject_to_world(depth_map, K, pose_c2w):
    ...
    pixel_homog = np.stack([xx, yy, np.ones_like(xx)], axis=-1)  # (H, W, 3)

    # Camera points
    cam_pts = np.einsum('ij,hwj->hwi', K_inv, pixel_homog)  # (H, W, 3)
    cam_pts *= depth_map[:,:, np.newaxis]  # (H, W, 3)

    # World points under original pose
    R_c2w = pose_c2w[:3,:3]
    t_c2w = pose_c2w[:3,3]

    pts3d_reproj = np.einsum('ij,hwj->hwi', R_c2w, cam_pts) + t_c2w[np.newaxis,np.newaxis,:]  # (H, W, 3)
    return pts3d_reproj #(H, W, 3)
