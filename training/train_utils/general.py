...
class AverageMeter:
    ...
    def update(self, val, n=1):
        ...
        if torch.is_tensor(val):
            value = val.detach().to(self.device)
        else:
            value = torch.tensor(float(val), device=self.device)
        count = torch.tensor(float(n), device=self.device)
        self.val = value
        self.sum = self.sum + value * count
        self.count = self.count + count
        self.avg = self.sum / self.count.clamp_min(1.0)
    ...
    def __str__(self) -> str:
        ...
        return fmtstr.format(
            name=self.name,
            val=self._as_float(self.val),
            avg=self._as_float(self.avg),
        )
    ...
    def value(self) -> float:
        ...
        return self._as_float(self.val)

    @property
    def average(self) -> float:
        """Get the running average."""
        return self._as_float(self.avg)
