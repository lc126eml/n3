...
def get_center_camera(K, pose, H=0, W=0, depthmap=None):
    # Do not use it! it will make pose non-rigid! 
    if depthmap is not None:
        H, W = depthmap.shape

    # Get original parameters and calculate shift
    fx, fy = K[0, 0], K[1, 1]
    Cx_orig, Cy_orig = K[0, 2], K[1, 2]
    Cx_new, Cy_new = (W - 1) / 2.0, (H - 1) / 2.0
    dx, dy = Cx_new - Cx_orig, Cy_new - Cy_orig
    if dx == 0 and dy == 0:
        return K, pose
    ...
    # Create corrective shear matrix
    M_inv = np.eye(4)
    M_inv[0, 2] = dx / fx
    M_inv[1, 2] = dy / fy
