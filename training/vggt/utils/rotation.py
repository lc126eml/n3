...
def safe_quat_to_mat(quaternions: torch.Tensor, eps: float = 1e-12) -> torch.Tensor:
    ...
    norm_quat = torch.nn.functional.normalize(quaternions, p=2, dim=-1, eps=eps)

    i, j, k, r = torch.unbind(norm_quat, -1)

    # Simplified formula for UNIT quaternions
    # This avoids the risky division
    ...
    o = torch.stack(
        (
            1 - 2 * (jj + kk),  # 1 - 2(j^2 + k^2)
            2 * (ij - kr),
            2 * (ik + jr),
            2 * (ij + kr),
            1 - 2 * (ii + kk),  # 1 - 2(i^2 + k^2)
            2 * (jk - ir),
            2 * (ik - jr),
            2 * (jk + ir),
            1 - 2 * (ii + jj),  # 1 - 2(i^2 + j^2)
        ),
        -1,
    )
