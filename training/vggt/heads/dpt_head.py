...
class DPTHead(nn.Module):
    ...
    def _forward_impl(
        ...
        preds, conf = activate_head(
            ...
            conf_logit_max=self.conf_logit_max,
        )
...
class DPTSharedHead(nn.Module):
    """Lightweight output head that consumes shared DPT features."""
    ...
    def __init__(
        ...
        self.output_conv = nn.Sequential(
            nn.Conv2d(feature_dim, head_features, kernel_size=3, stride=1, padding=1),
            nn.ReLU(inplace=True),
    ...
    def forward(self, features: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        out = self.output_conv(features.view(B * S, C, H, W))
        preds, conf = activate_head(
            out,
            activation=self.activation,
            conf_activation=self.conf_activation,
