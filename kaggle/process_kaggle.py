...
def _kernel_checkpoint_path(kernel_id: str) -> str | None:
    text = str(kernel_id or "").strip()
    if "/" not in text:
        return None
    owner, slug = text.split("/", 1)
    owner = owner.strip()
    slug = slug.strip()
    if not owner or not slug:
        return None
    return f"/kaggle/input/notebooks/{owner}/{slug}/logs/ckpts/checkpoint.pt"


def _dataset_checkpoint_path(dataset_id: str) -> str | None:
    ...
    if not owner or not slug:
        return None
    return f"/kaggle/input/datasets/{owner}/{slug}/logs/ckpts/checkpoint.pt"


def _effective_resume_checkpoint_path(cfg: dict):
    resume_checkpoint_path = cfg.get("resume_checkpoint_path")
    if resume_checkpoint_path is not None:
        return str(resume_checkpoint_path)
    ...
    if resume_source == "kernel":
        ...
        return _kernel_checkpoint_path(sources[0]) if sources else None
    if resume_source == "dataset":
        sources = _as_list(cfg.get("dataset_sources"))
        return _dataset_checkpoint_path(sources[0]) if sources else None
    return None
