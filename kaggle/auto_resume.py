...
def _kaggle_kernel_checkpoint_path(kernel_id: str) -> str | None:
    if "/" not in str(kernel_id):
        return None
    slug = str(kernel_id).split("/", 1)[1].strip()
    if not slug:
        return None
    return f"/kaggle/input/{slug}/checkpoint.pt"
...
def _build_resumed_cfg(
    ...
    *,
    ...
    resume_checkpoint_path = _kaggle_kernel_checkpoint_path(resumed_from)
